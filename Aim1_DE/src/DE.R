...
dds_subset <- DESeq(dds,parallel=TRUE, BPPARAM=MulticoreParam(4))
# resultsNames(dds_subset)

res <- results(dds_subset, contrast=c("CONDITION","Case","Control"),
               alpha = 0.05, parallel=TRUE, BPPARAM=MulticoreParam(4))
...
# load("../results/PPMI_CaseAll_CtrlNonCarrier_allgene_DEanalysis/DE/DESeq2_res.RData")
...
with(topT, plot(log2FoldChange, -log10(padj),
                pch=20, cex=0.5, main=prefix, col='gray',
...
EnhancedVolcano(res, lab = res$symbol, x = 'log2FoldChange', y = 'pvalue', drawConnectors = TRUE)
EnhancedVolcano(resLFC, lab = resLFC$symbol, x = 'log2FoldChange', y = 'pvalue', drawConnectors = TRUE)
