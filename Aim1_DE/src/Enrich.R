...
if(("symbol" %in% colnames(DE_result)) && (program=="all" || grepl("GO",program))) {
  ...
  ORA(DE_result$human_orth_symbol, allGenes=gene_names, nCutoff=3, pCutoff=0.05, output=file.path(output_dir,"all_gene"))
  if("log2FoldChange" %in% colnames(DE_result)){
    # UP regulated DE genes
    ORA(DE_result$human_orth_symbol[DE_result$log2FoldChange>0 ],
        allGenes=gene_names, output=file.path(output_dir,"all_gene.up"))
    # DOWN regulated DE genes
    ORA(DE_result$human_orth_symbol[DE_result$log2FoldChange<0],
        allGenes=gene_names, output=file.path(output_dir,"all_gene.down"))
  }
}
