...
scale_method = sys.argv[2] ## logz, logmm, mm
print("*******************************************")
########################
## train
...
#####################
# kept_features = DEGs + eRNAs + circRNAs + PRS_features + clinicals_features
feature_ls = DEGs + eRNAs + circRNAs + PRS_features + clinicals_features
train_X = train_data_df.loc[:,feature_ls]
...
for model in ["SVM","SVM_rbf","RG","LR","SGD","ABC","XGB","RF","KNN","MLP"]:
    output_dir = "../results/"+dataset+"/DT_SVM_pool_"+str(len(feature_ls)+4)+"_reselect/"+scale_method+"/" + model
    print(output_dir)
    ...
    if feature_selection.startswith("LASSO"):
        if os.path.exists(output_dir+"/../lasso_feature_coef.txt"):
            feature_df = pd.read_csv(output_dir+"/../lasso_feature_coef.txt", sep="\t")
            kept_features = list(feature_df.loc[feature_df["importance"] > 0, "gene"].values)
        else:
            print("Running lasso feature selection:")
            [kept_features, feature_df] = LASSO(train_i_X,train_i_Y)
            feature_df.to_csv(output_dir+"/../lasso_feature_coef.txt",sep="\t")
    elif feature_selection.startswith("DT"):
        if os.path.exists(output_dir+"/../dt_feature_importances.txt"):
            feature_df = pd.read_csv(output_dir+"/../dt_feature_importances.txt", sep="\t")
            kept_features = list(feature_df.loc[feature_df["importance"] > 0, "gene"].values)[:100]
        else:
            print("Running DT feature selection:")
            [kept_features, feature_df] = dt_func(train_i_X, train_i_Y,n=100)
            feature_df.to_csv(output_dir + "/../dt_feature_importances.txt", sep="\t")

    print("Selected feature number:", len(kept_features))
    # kept_features += PRS_features
    # kept_features += clinicals_features
    train_i_X =train_data_df.loc[train_index_ls,kept_features]
    train_i_X = train_i_X.loc[~train_i_X.isna().any(axis=1),:]
    train_i_Y = train_i_Y.loc[train_i_X.index.tolist(),:]
    train_index_ls =train_i_X.index.tolist()

    val_i_X =train_data_df.loc[val_index_ls,kept_features]
    val_i_X = val_i_X.loc[~val_i_X.isna().any(axis=1),:]
    val_i_Y = val_i_Y.loc[val_i_X.index.tolist(),:]
    val_index_ls = val_i_X.index.tolist()
   ...
   ## log transform
    ...
    if model=="RG" or model=="LR":
        ...
        coef_df.to_csv(output_dir+"/../"+model+"_coef.txt",sep="\t")
    else:
